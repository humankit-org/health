...
const jointFindings = [
    {
      // `mode: 'advanced'` (Phase 5.5): this finding references the Mayo
      // adiposity cluster total and would mislead in SIMPLE mode (where the
      // cluster does not exist). The base model never contains it — it is
      // only merged into the assembled advanced model — so the field is for
      // self-documentation + the data audit test (6.7e); the engine passes
      // it through, app.js does not filter on it.
      when: (v) => v.vo2maxOn, dir: 'good', input: 'VO2 max', source: ['weeldreyer2025'], mode: 'advanced',
    ...
    {
      when: (v) => v.heightCm > 0 && (v.weightKg / Math.pow(v.heightCm / 100, 2)) < 18.5, dir: 'neutral', input: 'Weight', source: ['sanchezlastra2021'], mode: 'advanced',
